...
def main():
    ...
    if (args.out != '-'):
        ...
        completion_time = pbar.format_dict['elapsed']
        with open('log_contents.txt', 'a') as output_file:
            output_file.write(str(completion_time))
        out.close()
        pbar.close()
    logging.info("Finished. Generated %d packets out of %d tries (%.3f)", f.good, f.tries, (f.good+0.0)/f.tries)
    log_contents = log_buffer.getvalue()
    with open('log_contents.txt', 'a') as output_file:
        output_file.write(log_contents)
    out.close()
    log_buffer.close()
    handler.close()
