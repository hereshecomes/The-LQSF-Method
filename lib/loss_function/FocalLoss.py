...
class Like_FocalLoss(nn.Module):
    def __init__(self, gamma=2):
        super(Like_FocalLoss,self).__init__()
        self.gamma = gamma
        self.bce = nn.BCELoss()
    def forward(self, preds, targets):
        bce_loss = self.bce(preds, targets)
        like_focal = torch.mul(torch.pow((targets-preds), self.gamma), bce_loss)
        loss = torch.mean(like_focal)
        return loss